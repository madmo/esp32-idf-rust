  ...
  outputs = { nixpkgs, ... }:
    let
      ...
      dockerArchive = pkgs.dockerTools.pullImage {
        ...
        imageDigest =
          "sha256:57b0b7c59288e4dcfa5143fe494ac4e145d3a78ff1e61cb772b78d17935aa051";
    ...
    in rec {
      packages.${system} = {
        ...
        toolchain-hook = pkgs.callPackage ({ }:
          pkgs.makeSetupHook {
            name = "toolchain-hook";
            propagatedBuildInputs = [ packages.${system}.toolchain ];
            substitutions = { shell = "${pkgs.bash}/bin/bash"; };
          } (pkgs.writeScript "xtensa-path-hook.sh" ''
            export LIBCLANG_PATH="${
              packages.${system}.toolchain
            }/.rustup/toolchains/esp/xtensa-esp32-elf-clang/esp-${esp_clang_version}/esp-clang/lib"
            export PATH="${
              packages.${system}.toolchain
            }/.rustup/toolchains/esp/bin:${
              packages.${system}.toolchain
            }/.rustup/toolchains/esp/xtensa-esp-elf/esp-${esp_gcc_version}/xtensa-esp-elf/bin:$PATH"
            export RUST_SRC_PATH="$(rustc --print sysroot)/lib/rustlib/src/rust/library"
          '')) { };
        default = packages.${system}.toolchain-hook;
      };
